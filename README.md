...
- **本地正文历史**：正文改动间隔满 5 分钟或一次改动较大时自动留存旧版本，每条笔记保留最近 20 条、最长 30 天；在文档信息里预览并一键恢复，恢复前会把当前正文也存进历史。只保存在本设备的浏览器里，不同步、不导出。
- **图片与 PWA**：粘贴图片上传到自己的服务端，双击或长按可在灯箱里放大、翻页、下载原图；已缓存的图片可离线查看；支持安装到桌面或手机主屏。
...
| 正文底部 | 查看字数、文档信息（含本地历史版本的预览与恢复）和 Markdown 格式提示 |
...
| 侧栏底部「设置」 | 切换主题（跟随系统 / 浅色 / 深色）、正文字号与编辑区宽度，只对当前设备生效 |
...
**应用内导出 / 导入**：侧栏底部的「导出与导入」可把全部笔记打包成 zip 下载，每个分组一个文件夹、每条笔记一个 `.md`，图片放在 `images/` 并改成相对路径，`index.json` 记录分组、星标、置顶、颜色与创建时间。同一份包可以导入回任意部署（Cloudflare 或独立服务器），图片会重新上传；重复导入不会产生副本。这是迁移和日常备份的推荐方式，不依赖命令行。
...
- [功能建议与待办](docs/roadmap.md) —— 待排期的功能与运维改进，按投入产出分三档
- [变更记录](CHANGELOG.md)
