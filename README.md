...
| 快捷键 | 功能 |
| --- | --- |
| `Ctrl / Cmd + Alt + N` | 新建笔记（`Ctrl + N` 是浏览器新窗口，网页拦不住） |
...
- **回收站过期清理**：默认关闭。设置 `TRASH_RETENTION_DAYS` 后，进入回收站超过该天数的笔记会被自动彻底删除，效果与手动「彻底删除」一致，删除信号照常同步到其他设备；开启后回收站页会提示保留天数，每条笔记显示还剩几天。Cloudflare 在 `wrangler.jsonc` 的 `vars` 里加 `"TRASH_RETENTION_DAYS": "30"` 后重新部署；独立服务器在 `.env.server` 或 `.env` 里设置同名变量后重启。值必须是正整数天数，留空或 `0` 表示关闭，非法值会让服务器拒绝启动。
