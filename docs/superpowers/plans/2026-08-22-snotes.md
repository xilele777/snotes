...
import { clearToken, getToken, hasToken, setToken, writeCookieHeader } from './token'
...
describe('token', () => {
  ...
  it('同时写一份 Cookie 供 <img> 使用，作用域限定在图片路径', () => {
    ...
    expect(document.cookie).toContain('snotes_token=abc')
    // Path 限定后 Cookie 只会随 /api/images/ 的请求发出，
    // 其余 API 仍然只认 Authorization 头
    expect(writeCookieHeader('abc')).toContain('Path=/api/images/')
    expect(writeCookieHeader('abc')).toContain('SameSite=Strict')
  })
  ...
  it('clearToken 同时抹掉 Cookie', () => {
    ...
    expect(document.cookie).not.toContain('snotes_token=abc')
  })
})
