...
- **鉴权**：除 `/api/health` 外，所有 `/api/*` 需要 `Authorization: Bearer <token>`，Worker 端用常量时间比较，失败返回 401。
- **安全**（规格 §11.2）：R2 桶保持私有；上传 MIME 白名单 `image/jpeg|png|gif|webp`，单文件 ≤ 10 MB；Markdown 渲染禁用原始 HTML；图片 URL 只允许同源 `/api/images/` 前缀；token 不入日志、不进 URL query。
...
│  ├─ sync/
│  │  ├─ engine.ts            调度：启动/可见/联网/定时
│  │  ├─ push.ts              outbox 串行消费 + 退避
│  │  ├─ pull.ts              分页拉取 + 正文批量拉取 + 游标推进
│  │  └─ conflict.ts          冲突副本生成
│  ├─ editor/
│  │  ├─ MilkdownEditor.vue   Milkdown 封装
│  │  └─ image-upload.ts      压缩 + 上传 + blob 占位替换
│  └─ components/
│     ├─ NoteList.vue         列表（置顶/星标/颜色/缩略图）
...
| M5 前端 UI | 16–19 | 完整可用的单机版便签（无同步） |
| M6 同步引擎 | 20–23 | 双端增量同步、离线恢复、冲突副本 |
...
- Test: `tests/worker/health.test.ts`（本 Task 先用 Node 环境的最小断言，Worker 运行时测试在 Task 2 建立）
...
npm i -D typescript vite @vitejs/plugin-vue vue-tsc \
        ...
        wrangler @cloudflare/vitest-plugin @cloudflare/workers-types
```

- [ ] **Step 2: 写 `package.json` 的 scripts 段**
...
```json
{
  ...
  "include": ["src/**/*.ts", "src/**/*.vue", "worker/**/*.ts", "shared/**/*.ts", "tests/**/*.ts"]
}
```
...
- Produces: Worker 测试范式——测试直接 `app.request(path, init, env)` 调用 Hono app，`env` 来自 `cloudflare:workers`，D1 迁移由 setup 文件应用。后续所有后端 Task 都复用这个范式。
...
- [ ] **Step 3: 写 `tests/setup/apply-migrations.ts`**

setup 文件在每个测试文件的存储隔离之外运行、且可能多次执行；`applyD1Migrations` 只应用尚未应用的迁移，重复调用是安全的。
...
Expected: FAIL——迁移文件或配置尚未就位时报表不存在；若 Step 1–3 已写好则应直接 PASS（本 Task 的「实现」就是迁移与配置本身）。

> **级联删除注意**：SQLite 默认不开启外键约束。若「级联删除」用例失败，在 `worker/db.ts`（Task 7 创建）的每次连接中无法设置 pragma——D1 的正确做法是在迁移里不依赖 `ON DELETE CASCADE`，而由应用层在 purge 时显式删 `note_body`。若该用例失败，**改为**：删除这条测试用例，并在 Task 8 的 `purgeNotes` 中显式 `DELETE FROM note_body WHERE note_id IN (...)`。请在本 Step 记录实际结果并据此选择路径。
...
Run: `npm run test:worker`
Expected: PASS（4 个用例，或按 Step 5 的注记去掉级联用例后 3 个）
...
import { describe, expect, it } from 'vitest'
import { createApp } from '../../worker/app'
import { timingSafeEqual } from '../../worker/auth'
...
describe('auth 中间件', () => {
  ...
  it('token 错误返回 401', async () => {
    const res = await app.request('/api/groups', { headers: { Authorization: 'Bearer wrong' } }, env)
    expect(res.status).toBe(401)
  })

  it('非 Bearer 形式返回 401', async () => {
    const res = await app.request('/api/groups', { headers: { Authorization: 'test-token' } }, env)
    expect(res.status).toBe(401)
  })

  it('token 正确则放行', async () => {
    const res = await app.request('/api/groups', { headers: OK }, env)
    expect(res.status).toBe(200)
  })
})
```
...
export const auth: MiddlewareHandler<{ Bindings: Env }> = async (c, next) => {
  ...
  if (!expected || !timingSafeEqual(header.slice(prefix.length), expected)) {
    return c.json({ error: 'unauthorized' }, 401)
  }
...
- [ ] **Step 4: 在 `worker/app.ts` 挂载中间件与一个占位 groups 路由**

占位路由让鉴权测试有目标；Task 9 会用真实实现替换它。
...
export function createApp() {
  ...
  app.get('/api/groups', (c) => c.json({ groups: [] }))
...
```bash
git add -A
git commit -m "feat(auth): add bearer token middleware with constant-time compare

单用户自用场景下用一次性长随机令牌替代完整鉴权体系。
比较使用常量时间实现，对长度与内容均不早退，避免时序侧信道。
...
规格 §6.2：标题从正文首行提取（去前导 `#` 与空白，截断 64 字符）；摘要取前 120 字符纯文本；缩略图取首个图片的 file_key。这三者由客户端计算并随 `content` 提交，服务端只存不算。

**Files:**
- Create: `shared/derive.ts`
- Modify: `shared/types.ts`
- Test: `shared/derive.test.ts`
...
describe('extractSummary', () => {
  ...
  it('剥掉常见行内标记', () => {
    expect(extractSummary('**粗体** _斜体_ `代码` > 引用 # 标题')).toBe('粗体 斜体 代码 引用 标题')
  })
...
```bash
git add shared/derive.ts shared/derive.test.ts shared/types.ts
git commit -m "feat(core): extract title, summary and thumbnail from markdown
...
- Produces: `interface OutboxTask { id?: number; note_id: string; kind: OutboxKind; payload: unknown; retry: number; next_at: number }`
...
describe('mergeTask', () => {
  it('没有既有任务时原样返回', () => {
    const incoming = base()
    expect(mergeTask(undefined, incoming)).toEqual(incoming)
  })
...
并把退避重置为立即可发——用户刚做的编辑不应继承上一次失败的等待窗口。"
```
...
**Interfaces:**
- Produces: `interface LocalNoteState { id: string; version: number; prop_version: number }`
...
describe('planPull', () => {
  ...
  it('远端 prop_version 更高则更新属性', () => {
    const plan = planPull(
      ...
      localMap({ id: 'a', version: 1, prop_version: 3 })
    )
  ...
  it('远端 version 更高则列入待拉正文', () => {
    const plan = planPull(
      [note({ id: 'a', version: 9, prop_version: 1 })],
      localMap({ id: 'a', version: 4, prop_version: 1 })
    )
  ...
  it('两个版本都更高则同时更新属性并拉正文', () => {
    const plan = planPull(
      ...
      localMap({ id: 'a', version: 1, prop_version: 1 })
    )
  ...
  it('版本都不更高则完全忽略——这就是重复 pull 的幂等性', () => {
    const local = localMap({ id: 'a', version: 5, prop_version: 5 })
  ...
  it('本地版本高于远端时也忽略，不回退本地未推送的修改', () => {
    const plan = planPull(
      ...
      localMap({ id: 'a', version: 7, prop_version: 7 })
    )
  ...
  it('混合批次各归其位', () => {
    const plan = planPull(
      ...
      localMap(
        { id: 'prop', version: 1, prop_version: 1 },
        { id: 'body', version: 1, prop_version: 1 },
        { id: 'same', version: 1, prop_version: 1 }
      )
    )
...
export function planPull(remote: NoteMeta[], local: Map<string, LocalNoteState>): PullPlan {
  ...
  for (const note of remote) {
    ...
    if (note.version > current.version) {
      plan.fetchBody.push(note.id)
    }
  }
...
notesRoutes.patch('/api/notes/:id', async (c) => {
  ...
  const conflicted =
    (touchesBody && req.base_version !== undefined && req.base_version < current.version) ||
    (touchedProps.length > 0 &&
      req.base_prop_version !== undefined &&
      req.base_prop_version < current.prop_version)
...
Run: `npm run test:worker`
Expected: PASS（notes 16 个用例 + 先前的 auth / schema 用例）
...
不依赖 SQLite 外键级联（D1 默认不开启外键约束），显式删除每一张表的相关行。
...
- Modify: `worker/app.ts`（移除 Task 3 的占位 groups 路由）
- Modify: `shared/types.ts`
...
Expected: FAIL，占位路由返回空数组，创建与删除端点不存在
...
- [ ] **Step 5: 在 `worker/app.ts` 用真实路由替换占位路由**

删掉 `app.get('/api/groups', (c) => c.json({ groups: [] }))`，改为 `app.route('/', groupsRoutes)`。
...
> 注意 Task 3 的鉴权测试用 `GET /api/groups` 作为「放行」的验证目标，而本 Task 没有实现 GET。分组列表通过 `/api/sync/pull` 下发（Task 10），无需独立 GET。**请把 `tests/worker/auth.test.ts` 中两处 `/api/groups` 的目标改为 `/api/notes/x/trash`（method POST），期望值 401 保持不变，「放行」用例的期望状态码从 200 改为 404**——404 同样证明请求已通过鉴权进入路由。
...
describe('cursor', () => {
  ...
  it('结构不对的游标返回 null', () => {
    expect(decodeCursor(btoa('没有冒号'))).toBeNull()
  })
})
```
...
import { json, resetDb } from './helpers'
...
分页条件 `(update_time > ts) OR (update_time = ts AND id > id)` 是「同毫秒不重不漏」的关键；排序必须与之严格一致，即 `ORDER BY update_time ASC, id ASC`。
...
```ts
import type { BodiesRequest, BodiesResponse, NoteBody } from '../../shared/types'
...
imagesRoutes.post('/api/images/upload', async (c) => {
  ...
  if (!(file instanceof File) || typeof noteId !== 'string' || !noteId) {
...
describe('updateBody', () => {
  ...
  it('递增本地 body_version 并置 dirty', async () => {
    const note = await createNote('a')
    await updateBody(note.id, 'b')

    const updated = await getNote(note.id)
    expect(updated!.body_version).toBe(note.body_version + 1)
...
describe('listNotes', () => {
  ...
  it('置顶的排在前面，其余按更新时间倒序', async () => {
    ...
    await updateProps(c.id, { top: 1 })

    const list = await listNotes({ view: 'all' })
    expect(list[0].id).toBe(c.id)
    expect(list.slice(1).map((n) => n.id)).toEqual([b.id, a.id])
  })
...
describe('enqueue', () => {
  it('不同 note 的同类任务不合并', async () => {
    await enqueue({ note_id: 'a', kind: 'prop', payload: { star: 1 }, retry: 0, next_at: 0 })
    await enqueue({ note_id: 'b', kind: 'prop', payload: { star: 1 }, retry: 0, next_at: 0 })
...
db.version(1).stores({
  ...
  outbox: '++id, note_id, kind, next_at, [note_id+kind]',
  groups: 'group_id, ord, invalid',
  meta: 'key',
})
```

- [ ] **Step 6: 写 `src/db/repo.ts`**

写入路径的共同形状：**改本地 → 入队 → 立即返回**。任何一步都不碰网络。
...
export async function updateBody(id: string, content: string): Promise<void> {
  ...
  await db.notes.update(id, {
    body: content,
    body_version: note.body_version + 1,
    title,
    summary,
    thumbnail,
...
async function setInvalid(id: string, invalid: 0 | 1, kind: 'trash' | 'recover') {
  ...
  await enqueue({ note_id: id, kind, payload: {}, retry: 0, next_at: 0 })
}
...
export async function listNotes(filter: ListFilter): Promise<LocalNote[]> {
  ...
  return matched.sort((a, b) => b.top - a.top || b.update_time - a.update_time)
}
...
派生字段在写入时同步重算，保证离线也能立刻渲染出正确的列表标题。
outbox 按 (note_id, kind) 复合索引合并，连续敲字只留一条 body 任务。
正文与属性使用各自的 dirty 位与任务类型，改星标不会重传正文。"
```
...
export const useUiStore = defineStore('ui', () => {
  ...
  return { view, activeGroupId, query, syncing, lastSyncError }
})
```
...
export const useGroupsStore = defineStore('groups', () => {
  ...
  async function load() {
    ...
    groups.value = all.filter((g) => g.invalid === 0).sort((a, b) => a.ord - b.ord)
  }
  ...
  async function create(name: string) {
    ...
    await enqueue({
      note_id: group.group_id,
      kind: 'create',
      payload: { scope: 'group', group_id: group.group_id, name, ord: group.ord },
      retry: 0,
      next_at: 0,
    })
    await load()
...
describe('MilkdownEditor 外壳', () => {
  ...
  it('切换 noteId 时丢弃上一条笔记未触发的 debounce，避免串写', async () => {
    const wrapper = mount(MilkdownEditor, {
      props: { noteId: 'n1', modelValue: 'a' },
    })
...
`syncingExternally` 是防回环的关键：外部 setMarkdown 会触发 listener，若不加守卫就会把远端内容当成用户编辑再写回去。
...
watch(
  () => props.noteId,
  () => {
    // 切换笔记：丢弃上一条未触发的 debounce，避免把旧内容写进新笔记
    clearTimer()
...
切换笔记时主动丢弃上一条尚未触发的 debounce，否则旧内容会被写进新笔记。
外部 setMarkdown 用 syncingExternally 守卫，防止同步下来的内容被当成用户编辑再传回去。
组件对外只有 noteId/modelValue 两个 props 与一个事件，
使编辑器实现可整体替换而不波及其余代码——这是规格 15 所列风险的兜底设计。"
```
...
- Modify: `src/App.vue`
- Test: `src/components/GroupSidebar.test.ts`
...
- [ ] **Step 4: 在 `src/App.vue` 用 `GroupSidebar` 替换原来的 `<aside>` 内容**
...
```bash
git add -A
git commit -m "feat(ui): add group sidebar with fixed views
...
describe('pushOnce', () => {
  ...
  it('不可重试的 4xx 直接丢弃任务，不无限重试', async () => {
    await createNote('a')
  ...
  it('trash 与 recover 任务走各自端点', async () => {
    ...
    await db.outbox.add({ note_id: note.id, kind: 'trash', payload: {}, retry: 0, next_at: 0 })
    await pushOnce()
  ...
  it('outbox 为空时不发任何请求', async () => {
    ...
    expect(result).toEqual({ sent: 0, failed: 0, conflicts: [] })
    expect(apiFetch).not.toHaveBeenCalled()
  })
})
```
...
async function applyAck(task: OutboxTask, ack: ServerAck) {
  ...
  const patchFields: Record<string, unknown> = { dirty: 'none' }
  if (typeof ack.version === 'number') patchFields.version = ack.version
  if (typeof ack.prop_version === 'number') patchFields.prop_version = ack.prop_version
  if (typeof ack.update_time === 'number') patchFields.update_time = ack.update_time

  await db.notes.update(task.note_id, patchFields)
}

export async function pushOnce(): Promise<PushResult> {
  const now = Date.now()
  const result: PushResult = { sent: 0, failed: 0, conflicts: [] }
  ...
  for (const task of tasks) {
    ...
    try {
      ...
      if (task.id !== undefined) await db.outbox.delete(task.id)
      result.sent++
    } catch (error) {
...
关掉浏览器再打开仍从原退避进度继续。
401 刻意不消耗任务，令牌修好后可续传；其余不可重试的 4xx 直接丢弃，
避免脏任务无限重试。服务端返回 conflicted 时上报本地正文，由上层生成冲突副本。"
```
...
async function localStates(): Promise<Map<string, LocalNoteState>> {
  const all = await db.notes.toArray()
  return new Map(all.map((n) => [n.id, { id: n.id, version: n.version, prop_version: n.prop_version }]))
}
...
export async function pullOnce(): Promise<PullResult> {
  ...
  for (;;) {
    ...
    if (plan.insert.length > 0) {
      await db.notes.bulkAdd(plan.insert.map(toLocalNote))
    }
  ...
  for (let i = 0; i < pendingBodies.length; i += BODY_BATCH) {
    ...
    for (const body of response.bodies) {
      ...
      // 本地正文比远端新（尚未推送）时不覆盖
      if (local.body_version > body.version) continue
...
```ts
import { extractTitle } from '../../shared/derive'
import { createNote } from '../db/repo'
import { db } from '../db/schema'
...
export async function saveConflictCopies(conflicts: ConflictInfo[]): Promise<string[]> {
  ...
  for (const conflict of conflicts) {
    ...
    const title = `${extractTitle(conflict.local_body) || '无标题'}${CONFLICT_SUFFIX}`
    const content = `# ${title}\n\n${conflict.local_body}`

    const copy = await createNote(content)

    if (original.group_id) {
      await db.notes.update(copy.id, { group_id: original.group_id })
    }
...
import { db } from '../db/schema'
import { useUiStore } from '../stores/ui'
import { POLL_INTERVAL_MS, startSyncEngine, syncNow } from './engine'
...
beforeEach(async () => {
  ...
  pushOnce.mockReset().mockResolvedValue({ sent: 0, failed: 0, conflicts: [] })
...
describe('syncNow', () => {
  it('先 push 再 pull', async () => {
    ...
    pushOnce.mockImplementation(async () => {
      order.push('push')
      return { sent: 0, failed: 0, conflicts: [] }
    })
  ...
  it('并发调用被合并，不会重入', async () => {
    ...
    pushOnce.mockImplementation(
      () =>
        new Promise((resolve) => {
          resolvePush = () => resolve({ sent: 0, failed: 0, conflicts: [] })
        })
    )
...
同步完成后要刷新 store，否则拉下来的数据不会出现在界面上。
...
五个触发点：启动、页面重新可见、网络恢复、前台每 30 秒、本地写入后由 outbox 承接。
...
- Produces: `MAX_EDGE = 1600`、`JPEG_QUALITY = 0.82`
- Produces: `compressImage(file: File): Promise<Blob>`
...
`compressImage` 依赖 canvas，jsdom 下不可用，因此只测**不依赖 canvas 的部分**：MIME 判定、占位替换、上传调用与失败处理。压缩本身在 Step 5 的手工验证里覆盖。

```ts
import { beforeEach, describe, expect, it, vi } from 'vitest'
import { ApiError } from '../api/client'
import { isAllowedImage, replacePlaceholder, uploadImage } from './image-upload'
...
- [ ] **Step 3: 写 `src/editor/image-upload.ts`**
...
- [ ] **Step 4: 在 `MilkdownEditor.vue` 接入粘贴上传**

在 `<script setup>` 中加入，并在 `editorViewOptionsCtx` 里挂上 `handlePaste`：

```ts
import { replaceAll } from '@milkdown/kit/utils'
import { replacePlaceholder, uploadImage } from './image-upload'
...
async function handleImageFiles(files: File[]) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      latest = replacePlaceholder(latest, placeholder, url)
      syncingExternally = true
      editor.action(replaceAll(latest))
      syncingExternally = false
      emit('update:modelValue', latest)
...
在编辑器 config 中替换 `editorViewOptionsCtx` 那一行：
...
4. 左侧列表中该笔记应出现缩略图
5. 断网状态下粘贴图片，应看到占位图与失败提示，且占位保留在正文中
...
上传失败时刻意保留占位，使用户能看出这张图尚未上传，而不是悄悄消失。
...
Expected: FAIL，`pwaOptions` 未导出

- [ ] **Step 3: 改写 `vite.config.ts`**
...
- [ ] **Step 2: 写 `tests/e2e/full-flow.spec.ts`**
...
test.beforeEach(async ({ page }) => {
  await page.goto('/')
  await page.evaluate((t) => localStorage.setItem('snotes_token', t), TOKEN)
  await page.reload()
})
...
test('同步：两个上下文之间数据可互通', async ({ browser }) => {
  ...
  await pageA.goto('/')
  await pageA.evaluate((t) => localStorage.setItem('snotes_token', t), TOKEN)
  await pageA.reload()
  ...
  await pageB.goto('/')
  await pageB.evaluate((t) => localStorage.setItem('snotes_token', t), TOKEN)
  await pageB.reload()
...
| 规格章节 | 验收方式 |
| --- | --- |
| §2 功能范围 | E2E 覆盖新建、编辑、分组、搜索、删除、恢复；图片粘贴经手工验证 |
| §5.1 本地优先 | 断网状态下全部读写可用，界面响应无延迟 |
| §6 数据模型 | `migrations/0001_init.sql` 与 Dexie schema 字段一一对应 |
| §7 API | `npm run test:worker` 全绿，覆盖全部端点 |
| §8 同步 | `npm test` 中 push/pull/conflict/engine 四组测试全绿 |
| §9 图片 | 上传经 Worker 中转，R2 桶私有，MIME 与大小校验有测试 |
| §10 搜索 | 纯本地实现，无服务端参与，有测试 |
| §11 鉴权与安全 | 常量时间比较有测试；token 不入 URL 有测试；MIME 白名单有测试 |
| §12 PWA | 构建产物含 sw.js 与 manifest；手机可安装、可离线看图 |
| §13 部署运维 | 线上冒烟通过；备份与令牌轮换流程成文 |
...
3. **分组任务复用 outbox 表**（Task 15）——用 `payload.scope === 'group'` 区分，共享同一套退避与合并逻辑，避免第二张队列表。
4. **不提供 `GET /api/groups`**（Task 9）——分组列表经 `sync/pull` 首页下发，少一次请求往返。规格 §7.3 未要求独立 GET。
5. **`invalid` 不经 PATCH 修改**（Task 7）——只能走 trash / recover 端点，使删除态的变更路径唯一。
