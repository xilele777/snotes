...
- [x] **Step 1: 写失败测试 `src/stores/notes.test.ts`**
...
- [x] **Step 4: 写 `src/stores/notes.ts`**
...
- [x] **Step 5: 写 `src/stores/groups.ts`**
...
- [x] **Step 1: 写失败测试 `src/components/NoteList.test.ts`**

> **实现偏离记录（2026-08-23）：** 计划原稿用 `item.trigger('pointerdown', { clientX: 100 })` 驱动左滑。
> 但 jsdom 29 下 `clientX` 是 `MouseEvent`/`PointerEvent` 原型上的只读 getter，
> `@vue/test-utils` 的 `createDOMEvent` 检测到无 setter 后会跳过赋值，`clientX` 永远为 0，
> 滑动 delta 算出来是 0，左滑测试无法展开删除按钮。
>
> 改为直接构造 `new PointerEvent(type, { clientX, bubbles: true })` 并 `el.dispatchEvent`，
> 把 `clientX` 传进 event init。已抽成 `pointer(el, type, clientX)` 辅助函数。
> 另外，模板里操作按钮的 `@click` 不 `await store.setProps`，store 回填 `notes` 与组件重渲染之间存在微任务时序差，
> 单纯 `nextTick`/`flushPromises` 仍可能抢跑（偶发断言时 store 还没回填）；
> 相关断言改用 `vi.waitFor` 等到状态落到 DOM/`store.notes` 后再校验，更稳健。
> 其余测试与计划一致。
...
- [x] **Step 3: 写 `src/components/NoteList.vue`**
...
- [x] **Step 4: 写 `src/components/TokenGate.vue`**
...
- [x] **Step 5: 写 `src/App.vue` 三栏骨架 + 移动端切换 + 新建按钮**
...
- [x] **Step 6: 写 `src/styles.css` 并在 `src/main.ts` 引入**
...
> **对照原站 note.wps.cn 修正布局（2026-08-23）：** Task 16–19 完成后对照原站实测产物
> （`docs/notes-wps-reference/`）发现三栏与列表项结构偏离原站，做以下修正（属 Task 16/19 视觉层后续修正，非新 Task）：
> 1. grid 由 `180px 1fr 1fr` 改为 `180px 360px 1fr`——列表区固定窄、编辑器占大头，贴合原站比例；1020px 断点改 `360px 1fr`。
> 2. NoteList 加 `list-header`（logo 名「snotes」+ 视图标题）——原站 logo 名在列表区 header 内（`header_name`），非独立全宽顶栏；原站 88px 顶部是 `migration_notice_spacer` 迁移通知占位，非常驻。
> 3. 列表项内部重构为原站 `note_list_item` 结构：缩略图 `float:right`、文字流式绕排、底部日期行（`note-meta`）含日期 + 常驻置顶/星标图标、分隔线 `note-divider` 缩进。移除原 `.note-acts`（hover 操作条）。
> 4. 置顶/星标改为底部日期行常驻图标（top/star=1 才显示）；颜色板、移至分组、删除迁到**编辑器顶栏「更多」菜单**（原站 `clz_more_icon_div`），App.vue 加 `more-menu` 弹出层 + 点外部关闭。
> 5. 侧栏分组项 hover 改 `--bg-press`(#efefef)、选中改 `--select-yellow`(#f4dc3d) 黄色高亮，对齐原站 `.group_item:hover`/`selected`。
> 6. NoteList.test.ts 移除已迁移的标记/颜色/分组用例，新增 header 与标记图标常驻用例；新建 `src/App.test.ts` 覆盖更多菜单（置顶/星标/颜色/分组下拉/删除）。
> 核查依据：原站 `html{font-size:100px}`（1rem=100px），`note_list_item` 高 `.93rem`、`note_list_item_thumb` `float:right` 48px、`note_list_bottom_line` `margin-left:88px`、`clz_more_icon_div` 顶栏更多按钮。
...
- [x] **Step 1: 写失败测试 `src/editor/MilkdownEditor.test.ts`**
...
- [x] **Step 3: 写 `src/editor/MilkdownEditor.vue`**

> **实现偏离记录（2026-08-23）：** 计划原稿把 `useEditor` 直接写在 `MilkdownEditor.vue` 顶层 `<script setup>` 里，
> 模板里 `<MilkdownProvider>` 包 `<Milkdown/>`。但 `@milkdown/vue@7.22` 的 `useEditor` 内部用
> `inject(editorInfoCtxKey)`，而该 context 是 `MilkdownProvider` 在**子组件** setup 里 `provide` 的——
> 祖先链上根本拿不到，`inject` 返回 `undefined`，`const { editorFactory, loading, editor } = inject(...)!` 直接崩，
> 编辑器实例从未创建，桌面端表现就是「完全无法输入」。单测因 `vi.mock('@milkdown/vue')` 把 `useEditor` 整个换掉而全绿，
> 掩盖了这个问题。
>
> 修复：把 `useEditor` + `<Milkdown/>` 抽到一个**内部子组件** `MilkdownInner`，再由外壳的
> `<MilkdownProvider>` 包裹。`useEditor` 此时在 provider 的后代里执行，注入成立。外壳通过
> `ref` 调用子组件 `expose` 出的 `replaceContent(md)`（内部走 `editor.action(replaceAll(...))`）来切换笔记内容，
> `watch(noteId)` 的 flush/换内容语义保持不变。`defineExpose({ onMarkdownChange })` 等外壳契约不变，Step 1 测试无需改动。
>
> 另追加 `.use(clipboard)`（`@milkdown/kit/plugin/clipboard`）：计划原稿 commonmark 只对逐字输入的 input rules 生效，
> 粘贴整段 markdown 文本会当纯文本插入不解析。clipboard 插件拦截 paste 用 schema 解析、复制时反向序列化。
> 实测粘贴 `# 标题\n- 列表` 正确生成 `<h1>` + `<ul><li>`。
...
- [x] **Step 4: 在 `src/App.vue` 用编辑器替换 `<pre>` 占位**
...
- [x] **Step 5: 在 `src/styles.css` 追加 Milkdown 渲染区样式**
...
> **iOS 真机验证暂缓（2026-08-23）：** 桌面端先发现并修复了 useEditor 注入断裂（无法输入）、ProseMirror 聚焦黑框与未撑满高度、内容贴边、粘贴 markdown 不解析等问题。iOS 真机验证（含中文输入法丢字等）延后到桌面端稳定后再做，届时若不达标按降级路径处理。本步骤未完成，仅记录暂缓状态。
...
- [x] **Step 1: 写失败测试 `src/components/GroupSidebar.test.ts`**
...
- [x] **Step 3: 写 `src/components/GroupSidebar.vue`**
...
- [x] **Step 4: 补 `src/components/NoteList.test.ts`——把笔记移进分组**
...
- [x] **Step 5: 在 `NoteList.vue` 加 `data-note-id` 与分组下拉**
...
- [x] **Step 6: 在 `src/App.vue` 用 `GroupSidebar` 注入第一栏**
...
- [x] **Step 7: 在 `src/styles.css` 追加侧栏样式**
...
- [x] **Step 1: 写失败测试 `src/components/SearchBar.test.ts`**
...
- [x] **Step 2: 写失败测试 `src/components/TrashView.test.ts`**

> **实现偏离记录（2026-08-23）：** 计划原稿 TrashView.test.ts 直接挂载组件测「清空」按钮。
> 但 TrashView 的 `cleanAll` 调 `window.confirm()`，jsdom 下 `confirm` 默认是 undefined
> （不像浏览器总是有），触发时抛 `TypeError: confirm is not a function`。
> 因此在 `beforeEach` 里 `vi.stubGlobal('confirm', () => true)` 先 stub 掉，
> 让清空流程在单测里能走通。组件代码不变——`confirm` 在真实浏览器与生产里行为照旧。
...
- [x] **Step 4: 写 `src/components/SearchBar.ts`**
...
- [x] **Step 5: 写失败测试 `src/components/shortcut.test.ts`**
...
- [x] **Step 7: 写 `src/components/shortcut.ts`**
...
- [x] **Step 8: 在 `NoteList.vue` 中用 `highlight` 渲染标题**
...
- [x] **Step 9: 写 `src/components/TrashView.vue`**
...
- [x] **Step 10: 在 `src/App.vue` 接入 TrashView 与全局快捷键**
...
- [x] **Step 11: 在 `src/styles.css` 追加高亮与回收站样式**
