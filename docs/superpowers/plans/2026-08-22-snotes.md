...
- Produces: `trashNote(id)`, `recoverNote(id)`, `listNotes(filter)`, `getNote(id)`
...
- Produces: `useUiStore()` —— `view: ListView`、`activeGroupId: string | null`、`query: string`、`syncing: boolean`、`lastSyncError: string | null`
...
export const useNotesStore = defineStore('notes', () => {
  ...
  return { notes, currentId, current, visible, load, create, saveBody, setProps, trash, recover }
})
```
...
- Produces: `NoteList.vue` —— props 无，直接读 store；emit `select(id: string)`
...
describe('NoteList', () => {
  ...
  it('颜色标记作为内联背景色渲染', async () => {
    const store = useNotesStore()
    const note = await store.create()
    await store.setProps(note.id, { skin_color: '#ffcc00' })

    const wrapper = mount(NoteList)
    await wrapper.vm.$nextTick()

    expect(wrapper.find('.note-item').attributes('style')).toContain('#ffcc00')
  })
  ...
  it('选色写入 skin_color，选「无」清回 null', async () => {
    const store = useNotesStore()
    const note = await store.create()
...
三个标记位（置顶 / 星标 / 颜色）的**读与写都在这里**——规格 §2 把它们列为要做的功能，光渲染不给入口等于没实现。写操作一律走 `notes.setProps`，于是自动获得 outbox 合并与 `prop_version` 语义。
...
/** null 排第一位，作为「清除颜色」 */
const COLORS = [null, '#ffd8a8', '#ffec99', '#b2f2bb', '#a5d8ff', '#eebefa'] as const
...
<template>
  <ul class="note-list">
    ...
    <li
      ...
      :key="note.id"
      class="note-item"
      :class="{ 'is-top': note.top === 1, 'is-active': note.id === notes.currentId }"
      :style="note.skin_color ? { backgroundColor: note.skin_color } : undefined"
      @click="notes.currentId = note.id"
    >
      <div class="note-text">
        <div class="note-title">
          <span v-if="note.star === 1" class="star">★</span>
          {{ note.title || '无标题' }}
        </div>
        <div class="note-summary">{{ note.summary }}</div>
      </div>

      <img v-if="note.thumbnail" :src="`/api/images/${note.thumbnail}`" alt="" />
...
<template>
  ...
  <div v-else class="layout">
    <aside class="sidebar">
      <button class="new-note" @click="notes.create()">新建笔记</button>
    </aside>
...
编辑区暂用 `<pre>` 占位，Task 17 替换为 Milkdown。
...
.layout { display: grid; grid-template-columns: 180px 280px 1fr; height: 100vh; }
.sidebar, .list-pane { border-right: 1px solid var(--border); overflow-y: auto; }
.editor-pane { overflow-y: auto; padding: 24px; }

.note-list { list-style: none; margin: 0; padding: 0; }
.note-item { display: flex; gap: 8px; padding: 12px; border-bottom: 1px solid var(--border); cursor: pointer; }
.note-item.is-active { background: #f0f6ff; }
.note-item.is-top .note-title::after { content: ' 置顶'; color: var(--muted); font-size: 12px; }
.note-item img { width: 48px; height: 48px; object-fit: cover; border-radius: 4px; }
.note-title { font-weight: 600; }
.note-summary { color: var(--muted); font-size: 13px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.empty, .placeholder { color: var(--muted); padding: 24px; }

.note-acts { display: flex; align-items: center; gap: 2px; opacity: 0; }
.note-item:hover .note-acts, .note-item.is-active .note-acts { opacity: 1; }
.note-acts button { border: 0; background: none; cursor: pointer; padding: 2px; font-size: 13px; }
.colors { display: flex; gap: 2px; }
.swatch { width: 12px; height: 12px; border-radius: 50%; border: 1px solid var(--border) !important; padding: 0 !important; }
.swatch.is-none { background: repeating-linear-gradient(45deg, #fff 0 3px, #ccc 3px 6px); }
...
.token-gate { max-width: 320px; margin: 15vh auto; display: grid; gap: 12px; }
...
`npm run dev`，输入任意令牌进入，点「新建笔记」，列表应立即出现一条「无标题」。刷新页面后该笔记仍在（IndexedDB 持久化）。

- [ ] **Step 9: 提交**

```bash
git add -A
git commit -m "feat(ui): add three-pane layout, token gate and note list

列表渲染置顶、星标、颜色与缩略图四个标记位，缩略图只拼接同源 /api/images/ 路径。
三个标记位同时提供操作入口，写操作走 setProps 复用 outbox 合并与 prop_version 语义。
令牌页在无令牌时接管整个界面，令牌一旦被 401 清空即自动回到此页。
窄屏下折叠为单栏，为手机 PWA 做准备。"
```
...
规格 §4.2、§8.2、§15。这是全项目唯一的真实技术风险点，本 Task 的最后一步是真机验证。
...
**Interfaces:**
- Produces: `MilkdownEditor.vue` —— props `{ noteId: string; modelValue: string }`，emit `update:modelValue`（已 debounce 800 ms）
...
同时在 `<script setup>` 里 `import MilkdownEditor from './editor/MilkdownEditor.vue'`。
...
`npm run dev`：新建笔记，输入 `# 标题` 应实时渲染为大字号标题；输入 `- ` 应变为列表；停止输入约 1 秒后，左侧列表标题应更新。

- [ ] **Step 7: iOS 真机验证（规格 §15 要求的风险验证，不可跳过）**
  ...
  <nav class="group-sidebar">
    ...
    <ul class="groups">
      ...
      >
        ...
        <button class="remove" title="删除分组" @click.stop="groups.remove(group.group_id)">×</button>
      </li>
    </ul>
...
- [ ] **Step 5: 在 `NoteList.vue` 的 `.note-acts` 里加分组下拉**
...
- [ ] **Step 6: 在 `src/App.vue` 用 `GroupSidebar` 替换原来的 `<aside>` 内容**
...
```bash
git add -A
git commit -m "feat(ui): add group sidebar and note-to-group assignment

侧栏同时承载全部/星标/回收站三个固定视图与用户分组，单层无嵌套。
...
- Modify: `src/App.vue`
- Modify: `src/components/NoteList.vue`
- Test: `src/components/SearchBar.test.ts`, `src/components/TrashView.test.ts`

**Interfaces:**
- Produces: `highlight(text: string, query: string): Array<{ text: string; hit: boolean }>`（`src/components/SearchBar.vue` 内导出的纯函数，供 NoteList 复用）
...
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { useUiStore } from '../stores/ui'
import SearchBar, { highlight } from './SearchBar.vue'
...
describe('SearchBar', () => {
  it('输入后 150ms 才写入 store', async () => {
    ...
    vi.advanceTimersByTime(1)
    expect(ui.query).toBe('关键词')
  })

  it('连续输入只写入最后一次', async () => {
    const ui = useUiStore()
    const wrapper = mount(SearchBar)
...
- [ ] **Step 4: 写 `src/components/SearchBar.vue`**

`highlight` 用 `indexOf` 循环而非正则，因此查询词里的 `.` `*` 等元字符按字面处理。
...
<template>
  <input v-model="input" class="search-bar" type="search" placeholder="搜索标题与正文" />
</template>
```

- [ ] **Step 5: 在 `NoteList.vue` 中用 `highlight` 渲染标题**
...
并在 `<script setup>` 里 `import { highlight } from './SearchBar.vue'` 与 `const ui = useUiStore()`。在 `src/styles.css` 加 `.hit { background: #ffe58f; }`。

- [ ] **Step 6: 写 `src/components/TrashView.vue`**
...
async function cleanAll() {
  ...
  const trashed = await db.notes.filter((n) => n.invalid === 1).toArray()
...
<template>
  <div class="trash-view">
    <button v-if="notes.notes.length > 0" class="clean-all" @click="cleanAll">清空回收站</button>

    <ul class="note-list">
      <li v-for="note in notes.notes" :key="note.id" class="note-item">
...
- [ ] **Step 7: 在 `src/App.vue` 接入**

在列表栏顶部放 `<SearchBar />`；当 `ui.view === 'trash'` 时用 `<TrashView />` 替代 `<NoteList />`。
...
```bash
git add -A
git commit -m "feat(ui): add local search with highlight and trash view

搜索完全在本地内存完成，输入 debounce 150 毫秒，无任何服务端参与，
因而离线可用且响应在毫秒级。命中片段用 indexOf 循环切分而非正则，
使查询词中的正则元字符按字面处理。
清空回收站需二次确认，并入队一条 purge 任务由同步引擎在服务端物理删除。"
```

---

**M5 完成检查点**：`npm test` 全绿，`npm run dev` 下应用完整可用——新建、编辑、分组、搜索、删除、恢复、清空全部可操作，全程不需要网络。这已经是一个可日常使用的单机便签。
