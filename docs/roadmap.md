...
- 状态：实施中。第一档 1、第二档 1 已于 v0.8.0 完成，第二档 2 已于 v0.9.0 完成，第一档 2 已于 v0.10.0 完成，第一档 3 已于 v0.11.0 完成，第一档 4 已于 v0.12.0 完成，其余待排期。
...
- 2026-09-19：v0.11.0 完成第一档 3「链接处理」：光标落在链接上时浮出气泡（打开、编辑地址、复制、移除），Ctrl/Cmd 加点击直接新标签打开，外链补 `rel="noopener noreferrer"`；新增 `src/editor/link-tooltip.ts`（ProseMirror Plugin，气泡挂 body 用 fixed 定位），`copyText` 从 `copyMarkdown` 中拆出复用。图片查看仍待做。
