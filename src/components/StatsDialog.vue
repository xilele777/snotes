...
import { closeStats, openStatsNote } from '../navigation'
...
useDialogFocus(() => props.open, panel, closeStats, () => {
...
<template>
  <Teleport to="body">
    <div v-if="open" class="dialog-mask stats-mask" @click.self="closeStats">
      <div ref="panel" class="dialog stats-dialog" role="dialog" aria-modal="true" aria-label="记录统计">
        <div class="stats-dialog-header">
          ...
          <button type="button" class="icon-button" aria-label="关闭统计" title="关闭统计" @click="closeStats"><AppIcon name="close" :size="20" /></button>
        </div>
